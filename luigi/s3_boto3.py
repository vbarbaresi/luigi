...
class S3Client(FileSystem):
    ...
    DEFAULT_THREADS = 100
    REFRESH_INTERVAL = datetime.timedelta(minutes=59)  # session ends after 1 hour: refresh after 59 min to be safe
    ...
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None, aws_session_token=None,
        ...
        # If we are in GovCloud, we need to force the boto3 client to use the FIPS endpoint,
        # unless explicitly overridden by user options
        if "DD_ENV" in os.environ and os.environ["DD_ENV"] == "gov":
            if not "region_name" in options and not "endpoint_url" in options:
                options["endpoint_url"] = "https://s3-fips.us-gov-west-1.amazonaws.com"

        # 'host' should only be used for boto2 so we need to remove it for boto3
        if "host" in options:
            options.pop("host")

        self._options = options
        # keep track of the current session date so we can refresh if necessary
    ...
    def s3(self):
        ...
        session_duration = datetime.datetime.now() - self.session_date
        if self._s3 and session_duration < self.REFRESH_INTERVAL:
            # reuse S3 client only if it's still usable (session duration < 1 hour)
            return self._s3
        self.session_date = datetime.datetime.now()
    ...
    def list_directory(self, path, start_time=None, end_time=None, return_key=False, **kwargs):
        """
        ...
        :param kwargs: Keyword arguments are passed to the boto function `objects.filter`
        """
        (bucket, key) = self._path_to_bucket_and_key(path)
        ...
        for item in s3_bucket.objects.filter(Prefix=key_path, **kwargs):
    ...
    def list(self, path, start_time=None, end_time=None, return_key=False, **kwargs):  # backwards compat
        key_path_len = len(self._add_path_delimiter(path))
        for item in self.list_directory(path, start_time=start_time, end_time=end_time, return_key=return_key, **kwargs):
            if return_key:
                yield item
            else:
...
class ReadableS3File(object):
    def __init__(self, s3_key):
        ...
        # set default buffer size to 8192 (python io default) instead of 1024 (botocore default)
        self.s3_key._DEFAULT_CHUNK_SIZE = io.DEFAULT_BUFFER_SIZE
        self.buffer = []
        self.closed = False
        self.finished = False
